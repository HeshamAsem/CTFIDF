...
WordsValue = {} # Global dictionary to store IDF scaling factors

def ModifiedCTFIDF1(Data: pd.DataFrame, N: int, Name: str):
    """
    Performs K-Means clustering and then extracts keywords using a modified CTF-IDF (version 1).
    This modification adjusts IDF based on the proportion of documents containing a word.
    ...
    """
    global WordsValue # Declare intention to use the global WordsValue dictionary
    ...
    def compute_idf(term_freq_documents: list[Counter], original_documents: list[str]):
        """
        Computes Inverse Document Frequency (IDF) with a modification.
        The modification scales IDF by the proportion of `original_documents`
        in which a term appears.
        """
        global WordsValue # Access the global dictionary to store scaling factors
        N_docs = len(term_freq_documents) # Number of cluster documents
        idf_dict = dict.fromkeys(term_freq_documents[0].keys(), 0) # Initialize IDF for all unique words
        ...
        L = [] # To store scaling factors for mean calculation
        # Calculate modified IDF
        for word, doc_count in idf_dict.items():
            # Calculate the scaling factor: (number of original documents containing word) / 5
            # The '5' here seems arbitrary and might need to be adjusted based on context
            scaling_factor = (len([2 for doc in original_documents if word in doc]) / 5)
            idf_dict[word] = math.log(1 + (N_docs / float(doc_count))) * scaling_factor
            L.append(scaling_factor)
            WordsValue[word] = np.round(scaling_factor, 2) # Store scaling factor globally

        print(f"Mean IDF Scaling Factor: {np.mean(L)}")
        return idf_dict
    ...
    idf = compute_idf(term_freq_docs, Documents) # Use 'Documents' as original_documents for scaling
    ...
    # This part calculates standard TF-IDF, which is also done in MainCTFIDF.
    # It might be redundant if the focus is purely on the custom CTF-IDF.
    # Keeping it for now as it was in the original code for comparison.
    ...
    # Iterate through each cluster to find and print top words from both methods
    for e, cluster_id, tfidf_scores_for_cluster in zip(range(len(Data['Predicted'].unique())),
        ...
        # Top words using standard CTFIDF (from TfidfVectorizer for comparison)
